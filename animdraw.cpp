...
//アニメーション描画
void AnimDraw::DrawAnimation(int apid, int x, int y, int count) {
	if (aplist.at(apid).ghlist.size() > 1) { //コマ数の確認
		int curpat = 0;
		curpat = count / aplist.at(apid).animgap % aplist.at(apid).ghlist.size();
...
//アニメーション左右(上下)反転描画
void AnimDraw::DrawTurnAnimation(int apid, int x, int y, int count, Direction dir) {
	if (aplist.at(apid).ghlist.size() > 1) { //コマ数の確認
		int curpat = count / aplist.at(apid).animgap % aplist.at(apid).ghlist.size();
...
//アニメーション回転描画
void AnimDraw::DrawRotaAnimation(int apid, int x, int y, int count, float rad) {
